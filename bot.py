...
def main_menu_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔮 Моя карта", callback_data=CATEGORY_CHART_CB)],
...
async def category_chart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = category_keyboard([
        ("🌡️ Насколько гармонична моя карта", HARMONY_CB),
        (f"🪐 Расширенный разбор карты, {FEATURE_PRICE['extended_natal']} ₽", EXTENDED_NATAL_CB),
    ])
    await context.bot.send_message(chat_id=query.message.chat_id, text="Что посмотреть по вашей натальной карте?", reply_markup=kb)
...
async def category_money(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=query.message.chat_id, text="Что посмотреть про деньги?", reply_markup=kb)
...
async def category_love(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = category_keyboard([
        ...
        ("💞 Совместимость, 199 ₽", COMPAT_CB),
    ])
    await context.bot.send_message(chat_id=query.message.chat_id, text="Что посмотреть про отношения?", reply_markup=kb)
